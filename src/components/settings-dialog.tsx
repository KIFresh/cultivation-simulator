...
export default function SettingsDialog({
  ...
  const fetchModels = async (index: number) => {
    ...
    if (provider.type !== "ollama" && !provider.apiKey && !provider.keyConfigured)
      return toast.error("请先填写 API Key，或在服务端已配置密钥");
    abortControllers.current[index]?.abort();
    const controller = new AbortController();
    abortControllers.current[index] = controller;
    setFetchingModels((previous) =>
      previous.map((value, current) => (current === index ? true : value))
    );
    try {
      const response = await authorizedFetch("/api/settings/list-models", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setAvailableModels((previous) =>
        previous.map((value, current) => (current === index ? models : value))
      );
      ...
      setFetchingModels((previous) =>
        previous.map((value, current) => (current === index ? false : value))
      );
    }
  };
        ...
        {!authorized ? (
          <div className="space-y-4 py-3">
            ...
            <Input
              ...
              autoFocus
            />
            <Button className="w-full" onClick={authorize} disabled={authorizing}>
              {authorizing ? (
                "验证中..."
              ) : (
                <>
          ...
          <div className="space-y-6 py-2">
            {loadingSettings ? (
              ...
              providers.map((provider, index) => (
                <div key={index} className="space-y-3 border-b border-border pb-4 last:border-0">
                          ...
                          <div className="flex gap-1">
                            <div className="relative flex-1">
                              <Input
                                ...
                                value={
                                  provider.apiKey || (provider.keyConfigured ? "********" : "")
                                }
                                onChange={(event) =>
                                  updateProvider(index, "apiKey", event.target.value)
                                }
                                placeholder={
                                  provider.keyConfigured ? "已配置（留空保持不变）" : "sk-..."
                                }
                                className="pr-10"
                                disabled={provider.clearKey}
                              />
                              <button
                                type="button"
                                onClick={() =>
                            ...
                            {provider.keyConfigured && (
                              <Button
                                ...
                                variant="outline"
                                onClick={() =>
                                  updateProvider(index, "clearKey", !provider.clearKey)
                                }
                              >
                                {provider.clearKey ? "取消清除" : "清除"}
                              </Button>
                            )}
                          </div>
                          {provider.clearKey && (
                            <p className="text-xs text-destructive">保存后将永久清除该 API Key。</p>
                          )}
                        </div>
                      )}
                      <div className="space-y-1">
            ...
            <div className="flex items-center justify-between py-3 border-t border-border">
              ...
              <button
                ...
                className={`relative w-11 h-6 rounded-full ${devMode ? "bg-primary" : "bg-muted"}`}
              >
                <span
                  className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${devMode ? "translate-x-5" : ""}`}
                />
              </button>
            </div>
            <Button
              className="w-full"
              onClick={handleSave}
              disabled={loading || loadingSettings || !dirty}
            >
