...
describe('game-store', () => {
  ...
  describe('setCultivator', () => {
    it('should reset all derived fields when data is null', () => {
      useGameStore.setState({ gold: 500, location: 'school', inventory: [{ itemId: 'herb_qi', quantity: 1, equipped: false }] });
  ...
  describe('loadCultivator', () => {
    ...
    it('should fetch cultivator data and update state', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          cultivator: { id: 'c1', name: '测试', realm: '凡人', age: 10, gold: 200, stamina: 30, location: 'school' },
        }),
      });

      await useGameStore.getState().loadCultivator('user-1');
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/cultivator?userId=user-1'));
    });
  ...
  describe('performAction', () => {
    ...
    it('should set loading state, fetch, and clear loading', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({ narrative: { title: '修炼', narrative: '你静心修炼', mood: '专注' }, cultivator: mockCultivatorData }),
      });
  ...
  describe('breakthrough', () => {
    ...
    it('should call breakthrough API', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        headers: new Headers({ 'content-type': 'application/json' }),
        json: async () => ({ canBreakthrough: true, cultivator: { ...mockCultivatorData, realm: '炼气期' } }),
      });

      await useGameStore.getState().breakthrough();
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/breakthrough'), expect.any(Object));
    });
  });
  ...
  describe('retryNarrative', () => {
    ...
    it('should retry the last request', async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: async () => ({ narrative: { title: 'retry', narrative: 'retry text', mood: 'calm' } }),
      });
  ...
  describe('setLocation', () => {
    ...
    it('should call PATCH API when userId is set', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith(expect.stringContaining('/api/cultivator'), expect.objectContaining({
        method: 'PATCH',
      }));
    });
  });
