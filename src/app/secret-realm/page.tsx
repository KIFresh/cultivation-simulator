...
export default function SecretRealmPage() {
  ...
  const explore = async (realm: SecretRealm) => {
    ...
    if ((cultivator?.stamina ?? 0) < 10) {
      toast.error("体力不足，无法探索");
      return;
    }
    setBusy(realm.id);
    try {
      ...
      if (data.outcome === "success") {
        ...
        if (data.gainedAttr)
          parts.push(`${data.gainedAttr.attr} +${data.gainedAttr.after - data.gainedAttr.before}`);
        ...
        <Card className="border-border bg-card shadow-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-foreground flex items-center gap-2">
              <Mountain className="w-5 h-5 text-primary" /> 秘境钥匙
            </CardTitle>
          </CardHeader>
              ...
              <Card key={r.id} className="border-border bg-card shadow-md">
                <CardContent className="p-3 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-[11px] px-1.5 py-0.5 rounded ${r.danger === "高" ? "bg-red-100 text-red-600" : r.danger === "中" ? "bg-amber-100 text-amber-600" : "bg-green-100 text-green-600"}`}
                    >
                      凶险 {r.danger}
                    </span>
                  </div>
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    需 {r.reqRealm} 及以上 · 消耗体力 {cost}
                  </p>
                  <Button
                    size="sm"
                    className="w-full bg-primary hover:bg-[#B33A2A] text-white"
                    disabled={busy !== null || !reqOk || cultivator.stamina < cost}
                    onClick={() => explore(r)}
                  >
                    {busy === r.id ? "探索中..." : "入秘境探索"}
                  </Button>
