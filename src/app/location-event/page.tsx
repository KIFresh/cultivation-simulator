...
function effectHints(fx: LocationEventEffect, isAwake: boolean): string[] {
  const out: string[] = [];
  if (fx.goldDelta)
  ...
  if (fx.attrExp)
    for (const [k, v] of Object.entries(fx.attrExp))
      out.push(`📖 ${k === "spirit" && isAwake ? "灵性" : ATTR_LABEL[k] ?? k}经验 +${v}`);
        ...
        {!loading && !done && event && (
          <div className="rounded-2xl border border-[#234155] bg-[#0f1c26]/70 p-5 shadow-[0_0_40px_-10px_rgba(240,180,120,0.25)]">
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {effectHints(event.effects, isAwake).map((h, i) => (
                <span
                  key={i}
        ...
        {done && result && (
          <div className="mt-2 rounded-2xl border border-[#234155] bg-[#0f1c26]/70 p-5">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {effectHints(result.event.effects, isAwake).map((h, i) => (
                <span
                  key={i}
