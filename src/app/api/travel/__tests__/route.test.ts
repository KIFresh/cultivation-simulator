...
describe("Travel API — 夺宝闭环", () => {
  it("非坊市离开不触发夺宝", async () => {
    mockRequireCultivator.mockResolvedValue({
      cultivator: { ...baseCultivator, location: "wild" },
    });
  ...
  it("概率边界：今日已夺宝过不触发", async () => {
    ...
    mockRequireCultivator.mockResolvedValue({
      cultivator: {
        ...baseCultivator,
        location: "market",
        inventory: JSON.stringify([{ itemId: "spirit_sword", quantity: 1, equipped: false }]),
        milestones: JSON.stringify({ robDate: key, robCount: 1 }),
      },
    });
    const req = new NextRequest(new URL("http://test/api/travel"), {
      method: "POST",
