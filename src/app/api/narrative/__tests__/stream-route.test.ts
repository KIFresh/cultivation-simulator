...
const h = vi.hoisted(() => ({
  cultivator: {
    ...
    maxAge: null, bonusAge: 0, reincarnationCount: 0, talents: null,
    ...
    occupation: null, gender: null, schoolRank: 0, storySummaryUpdatedAt: null, storyEntries: null, storyEntriesUpdatedAt: null,
  ...
  prisma: {
    ...
    $transaction: vi.fn((tx: any) => {
      ...
      return tx({
        ...
        familyMember: { createMany: vi.fn(() => Promise.resolve({ count: 2 })), deleteMany: vi.fn(() => Promise.resolve({ count: 0 })) },
      });
    }),
  },
}));
...
vi.mock('@/lib/narrative', () => ({
  generateDailyCultivationNarrative: vi.fn(() => ({ type: 'DAILY_CULTIVATION', title: '修炼', narrative: '今日修炼', mood: '静', hint: '继续', summary: '今日修炼总结' })),
  generateBreakthroughNarrative: vi.fn(() => ({ type: 'BREAKTHROUGH', title: '突破', narrative: '成功突破', mood: '燃', hint: '恭喜', summary: '突破总结' })),
  generateEncounterNarrative: vi.fn(() => ({ type: 'ENCOUNTER', title: '意外', narrative: '发现', mood: '奇', summary: 's', choices: [{ text: '探查', risk: 'low', hint: '稳' }] })),
  generateBirthNarrative: vi.fn(() => ({ type: 'BIRTH', title: '出世', narrative: '降生', mood: '奇', summary: 's', suggestedName: '李逍遥' })),
  NarrativeError: class NarrativeError extends Error { code: string; constructor(m: string, code = 'E') { super(m); this.code = code; } },
  callAIStream: vi.fn(async function* () { yield '天地'; yield '灵机'; }),
  callAI: vi.fn(),
...
describe('Narrative API 流式契约（提交优先 + 可重试）', () => {
  ...
  it('BIRTH：committed 含 gameEventId', async () => {
    const res = await POST(makeStreamRequest({ userId: 'user1', type: 'BIRTH', worldName: '地球', identityName: '书香门第', worldId: 'earth' }));
    expect(res.headers.get('Content-Type')).toContain('event-stream');
    expect(captured.committed.gameEventId).toBeDefined();
  });
