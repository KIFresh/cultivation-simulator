...
export async function POST(request: NextRequest) {
  try {
    ...
    if ((cultivator.stamina ?? 0) < STAMINA_COST) return apiError(`体力不足（需要${STAMINA_COST}）`, 400);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (success) {
        ...
        if (existingProduct) {
          ...
          inventory.push({ itemId: productId, name: `${formula.productName}`, quantity: 1, type: "pill" });
        }

        product = { itemId: productId, name: `${formula.productName}（${qualityName(quality)}）`, quantity: 1 };
...
async function autoUnlockNextFormula(tx: any, cultivatorId: string, currentUnlocked: string[], difficulty: number) {
  // 同难度配方在首次炼丹后自动解锁
  const allFormulas = getAllFormulas();
  const sameDifficulty = allFormulas.filter(f => f.difficultyLevel === difficulty && !currentUnlocked.includes(f.id));
