...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    properties: json.properties(auth.cultivator.properties),
    defs: PROPERTY_DEFS,
  });
}
  ...
  switch (action) {
    case "buy": {
      ...
      const newProp = {
        ...
        selfLiving: false,
        rented: false,
        rentPrice: def.rentPrice,
        furniture: [],
      };
      props.push(newProp);
      await prisma.cultivator.update({
        where: { id: c.id },
        data: { properties: JSON.stringify(props), gold: { decrement: def.price } },
      });
    ...
    case "sell": {
      ...
      await prisma.cultivator.update({
        where: { id: c.id },
        data: { properties: JSON.stringify(props), gold: { increment: sellPrice } },
      });
    ...
    case "move_in": {
      ...
      // 迁出当前居住的房产
      props.forEach((p: any) => {
        if (p.selfLiving) p.selfLiving = false;
      });
