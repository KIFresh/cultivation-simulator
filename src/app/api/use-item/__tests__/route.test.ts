...
const mockApiError = vi.hoisted(() => vi.fn((msg: string, status: number) =>
...
describe('UseItem API - POST', () => {
  beforeEach(() => {
    ...
    mockApiError.mockImplementation((msg: string, status: number) =>
      new Response(JSON.stringify({ error: msg }), { status }),
    );
  });
  ...
  it('背包无该物品返回 400', async () => {
    mockGetItemById.mockReturnValue({ id: 'pill', useEffect: { type: 'recoverStamina', value: 20 } });
  ...
  it('recoverStamina 恢复体力', async () => {
    mockGetItemById.mockReturnValue({ id: 'stamina_pill', useEffect: { type: 'recoverStamina', value: 20 } });
    const c = makeCultivator({ stamina: 10, inventory: JSON.stringify([{ itemId: 'stamina_pill', quantity: 2, equipped: false }]) });
    mockRequireCultivator.mockResolvedValue({ cultivator: c });
    mockPrisma.cultivator.update.mockResolvedValue({ ...c, stamina: 30, inventory: JSON.stringify([{ itemId: 'stamina_pill', quantity: 1, equipped: false }]) });
  ...
  it('boostAttr 提升属性', async () => {
    mockGetItemById.mockReturnValue({ id: 'root_pill', useEffect: { type: 'boostAttr', value: 5, targetAttr: 'root' } });
    const c = makeCultivator({ attributes: '{"root": 10}', inventory: JSON.stringify([{ itemId: 'root_pill', quantity: 1, equipped: false }]) });
    mockRequireCultivator.mockResolvedValue({ cultivator: c });
    mockPrisma.cultivator.update.mockResolvedValue({ ...c, attributes: '{"root": 15}', inventory: '[]' });
  ...
  it('未鉴权返回 401', async () => {
    mockRequireCultivator.mockResolvedValue({ error: new Response(JSON.stringify({ error: 'AUTH' }), { status: 401 }) });
    const res = await POST(makeRequest({ userId: 'u1', itemId: 'pill' }));
