...
describe("Cultivator UseItem API - POST", () => {
  ...
  it("背包无该物品返回 400", async () => {
    ...
    mockPrisma.user.findUnique.mockResolvedValue({
      id: "u1",
      cultivator: makeCultivator({ inventory: "[]" }),
    });
  ...
  it("addExp 类型物品增加修炼值", async () => {
    mockGetItemById.mockReturnValue({
      id: "exp_pill",
      name: "修炼丹",
      useEffect: { type: "addExp", value: 50 },
    });
    const c = makeCultivator({
      cultivationExp: 100,
      totalExp: 200,
  ...
  it("learnTechnique 已掌握功法返回 400", async () => {
    ...
    mockTechniques["sword_foundation"] = { name: "剑道基础" };
    mockPrisma.cultivatorTechnique.findFirst.mockResolvedValue({
      id: "t1",
      techniqueId: "sword_foundation",
    });
    const c = makeCultivator({
      inventory: JSON.stringify([{ itemId: "ancient_tome", quantity: 1, equipped: false }]),
    });
