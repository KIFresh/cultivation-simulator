...
const mockCreateEntry = vi.hoisted(() => vi.fn((title: string, text: string, _important: boolean) => ({
  title, text, important: true, createdAt: new Date(),
})));
...
const makeCultivator = (overrides: any = {}) => ({
  ...
  cultivationExp: 0, totalExp: 0, age: 18, location: 'home',
...
describe('Cultivator API - POST 创建修炼者', () => {
  ...
  it('用户名已存在返回 409', async () => {
    ...
    const res = await POST(makeRequest({
      userName: 'existing', cultivatorName: '小刚', spiritualRoot: '火灵根',
    }));
    expect(res.status).toBe(409);
  });
...
describe('Cultivator API - GET 获取修炼者信息', () => {
  ...
  it('获取修炼者信息成功', async () => {
    ...
    mockPrisma.user.findUnique.mockResolvedValue({ id: 'u1', name: 'user1', cultivator: { ...c, events: [] } });
