...
describe("Techniques API - GET", () => {
  beforeEach(() => {
    ...
    mockPrisma.cultivatorTechnique.findMany.mockResolvedValue([
      { techniqueId: "basic_breathing", level: 1 },
    ]);
  });

  it("返回功法列表", async () => {
    const req = {
      url: "http://localhost/api/cultivator/techniques",
      nextUrl: { searchParams: new URL("http://localhost/api/cultivator/techniques").searchParams },
    } as NextRequest;
