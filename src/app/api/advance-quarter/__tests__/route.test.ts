...
describe('POST /api/advance-quarter — 健康 ≤ 0', () => {
  it('非跨年 health=0 时施加 injuryDebuff 且不恢复健康', async () => {
    ...
    vi.mocked(prisma.cultivator.updateMany).mockResolvedValue({ count: 1 });
    vi.mocked(prisma.cultivator.findUnique).mockResolvedValue({ ...fakeCultivator, health: 0, injuryDebuff: 0 });
    const res = await POST(makeRequest());
    expect(res.status).toBe(200);
    const updates = vi.mocked(prisma.cultivator.updateMany).mock.calls[0][0];
    expect(updates.data.health).toBe(0);
  ...
  it('非跨年 health 为负值时同样施加 debuff', async () => {
    ...
    vi.mocked(prisma.cultivator.updateMany).mockResolvedValue({ count: 1 });
    vi.mocked(prisma.cultivator.findUnique).mockResolvedValue({ ...fakeCultivator, health: -5, injuryDebuff: 1 });
    const res = await POST(makeRequest());
    expect(res.status).toBe(200);
    const updates = vi.mocked(prisma.cultivator.updateMany).mock.calls[0][0];
  ...
  it('跨年且原有 debuff>0 时按年递减（覆盖 zeroDebuff）', async () => {
    ...
    vi.mocked(prisma.cultivator.updateMany).mockResolvedValue({ count: 1 });
    vi.mocked(prisma.cultivator.findUnique).mockResolvedValue({ ...fakeCultivator, quarter: 4, health: 0, injuryDebuff: 5 });
    const res = await POST(makeRequest());
    expect(res.status).toBe(200);
    const updates = vi.mocked(prisma.cultivator.updateMany).mock.calls[0][0];
    expect(updates.data.health).toBe(0);
  ...
  it('跨年且原有 debuff=0 时叠加 zeroDebuff', async () => {
    ...
    vi.mocked(prisma.cultivator.updateMany).mockResolvedValue({ count: 1 });
    vi.mocked(prisma.cultivator.findUnique).mockResolvedValue({ ...fakeCultivator, quarter: 4, health: 0, injuryDebuff: 0 });
    const res = await POST(makeRequest());
    expect(res.status).toBe(200);
    const updates = vi.mocked(prisma.cultivator.updateMany).mock.calls[0][0];
    expect(updates.data.health).toBe(0);
