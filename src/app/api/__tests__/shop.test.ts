...
vi.mock('@/lib', () => ({
  getShopItems: vi.fn(),
  getItemById: vi.fn(),
  getRealmIndex: (r: string) => ["凡人", "炼气期", "筑基期", "结丹期", "元婴期", "化神期"].indexOf(r),
...
const mockCultivator = {
  ...
  inventory: '[]', stamina: 80, age: 16, realm: '凡人', location: 'home',
} as any;

const qiPillListing = {
  itemId: 'qi_pill', price: 10, minRealm: undefined,
  item: { id: 'qi_pill', name: '益气丹', icon: '💊', description: '补充灵气的基础丹药', effect: '修炼值+20' },
};
  ...
  describe('GET /api/shop', () => {
    ...
    it('境界不足的商品标注 locked', async () => {
      mockShopItems.mockReturnValue([
        qiPillListing,
        { itemId: 'spirit_sword', price: 120, minRealm: '筑基期', item: { id: 'spirit_sword', name: '灵剑', icon: '🗡️', description: '' } },
      ]);
      const req = new NextRequest(new URL('http://test/api/shop?realm=凡人'));
      const res = await GET(req);
      const data = await res.json();
