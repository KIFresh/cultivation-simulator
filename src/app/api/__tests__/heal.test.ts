...
function mockUpdate() {
  vi.mocked(prisma.cultivator.update).mockImplementation(
    (async (args: any) => {
      const data = args.data;
      const merged: any = { ...fakeCultivator };
      for (const k of Object.keys(data)) {
        const v = data[k];
        if (v && typeof v === 'object' && 'decrement' in v) merged[k] = (fakeCultivator as any)[k] - v.decrement;
        else if (v && typeof v === 'object' && 'increment' in v) merged[k] = (fakeCultivator as any)[k] + v.increment;
        else merged[k] = v;
      }
      return merged;
    }) as any,
...
describe('Heal API (P1#5)', () => {
  ...
  it('未鉴权返回 401', async () => {
    vi.mocked(requireCultivator).mockResolvedValueOnce({ error: NextResponse.json({ error: 'AUTH' }, { status: 401 }) });
    const res = await POST(req({ userId: 'u1', mode: 'rest' }));
