...
interface ShopItemData { itemId: string; price: number; category: string; minRealm?: string; locked?: boolean; lockReason?: string; item: { id: string; name: string; icon: string; description: string; effect?: string } }
      ...
      <div className="mx-auto max-w-4xl px-4 py-6">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" className="text-gray-500 hover:text-amber-700" onClick={() => router.push("/dashboard")}>
              <ArrowLeft className="h-5 w-5" />
            </Button>
            <div>
              <h1 className="text-xl font-bold text-[#2C1E1E]">
                {isMarket ? "坊市" : "商店"}
              </h1>
              <p className="text-xs text-gray-400">
                {isMarket ? "风险与机遇并存" : "按需选购"}
              </p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          categories.map((cat) => (
            <div key={cat} className="mb-8">
              <h2 className="mb-4 text-sm font-bold uppercase tracking-widest text-gray-500">{cat}</h2>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {items.filter((i) => i.category === cat).map((s) => {
                  const isLocked = s.locked && !isMarket;
                  return (
                    <Card key={s.itemId} className={`overflow-hidden rounded-2xl border ${isLocked ? "border-gray-200 opacity-55" : "border-[#EADCD0]"} bg-white shadow-sm`}>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            {isLocked && s.lockReason && (
                              <p className="text-[10px] font-medium text-red-500">{s.lockReason}</p>
                            )}
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-bold text-primary">{s.price}💰</p>
                          <Button size="sm" className="h-7 text-xs bg-primary hover:bg-[#B33A2A] text-white mt-1" disabled={isLocked || gold < s.price} onClick={() => buy(s.itemId)}>
                            {isLocked ? "锁定" : "购买"}
                          </Button>
                        </div>
                      </CardContent>
