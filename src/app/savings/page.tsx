...
export default function SavingsPage() {
  ...
  const operate = async (action: "deposit" | "withdraw") => {
    ...
    try {
      ...
      toast.success(
        action === "deposit" ? `💰 存入 ${data.amount} 灵石` : `💰 取出 ${data.amount} 灵石`
      );
    } catch (e) {
        ...
        <Card className="border-border bg-card shadow-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-foreground flex items-center gap-2">
              <PiggyBank className="w-5 h-5 text-primary" /> 储蓄罐
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button
                className="bg-primary hover:bg-[#B33A2A] text-white"
                disabled={busy || cultivator.gold < amount}
                onClick={() => operate("deposit")}
              >
                存入
              </Button>
              <Button
                ...
                disabled={busy || cultivator.savings < amount}
                onClick={() => operate("withdraw")}
              >
                取出
              </Button>
            </div>
            <p className="text-[11px] text-muted-foreground text-center">
              储蓄罐里的灵石只在跨年时悄然生息，平日安稳不动。
            </p>
          </CardContent>
        </Card>
      </div>
