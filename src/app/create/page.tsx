...
export default function CreatePage() {
  ...
  const talentCost = selectedTalentIds.reduce(
    (sum, id) => sum + (TALENTS.find((t) => t.id === id)?.cost || 0),
    0
  );
  ...
  const handleCreate = async () => {
    ...
    try {
      const rootId =
        selectedRoot.element === "chaos"
          ? "chaos"
          : `${selectedRoot.element}_${selectedRoot.quality}`;
      ...
      const genNarrative = async (): Promise<{
        ok: boolean;
        suggestedName?: string;
        family?: any[];
        cultivator?: { name: string };
      }> => {
        setStreamingText("");
        try {
  ...
  const handleReset = () =>
    setAttributes({ root: 0, spirit: 0, insight: 0, luck: 0, charm: 0, mind: 0 });
  const handleBalance = () => {
          ...
          <div className="w-full max-w-lg mx-auto space-y-4">
            ...
            <div className="flex items-center gap-1 justify-center">
              {steps.map((s, i) => (
                <div key={i} className={`flex items-center gap-1 ${i > 0 ? "ml-0.5" : ""}`}>
                  ...
                  {i < steps.length - 1 && (
                    <div className={`w-4 h-0.5 ${i < step ? "bg-[#D9A13C]/40" : "bg-gray-200"}`} />
                  )}
                </div>
              ))}
            </div>
            ...
            {step === 0 && (
              <div className="space-y-2">
                {WORLDS.map((w) => (
                  <ZenCard
                    key={w.id}
                    selected={selectedWorld?.id === w.id}
                ...
                <div className="flex gap-2 pt-2">
                  <ZenButton variant="outline" onClick={goBack}>
                    返回
                  </ZenButton>
                  <ZenButton disabled={!selectedWorld} onClick={goNext}>
                    下一步
                  </ZenButton>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {BIRTHS.map((b) => {
                  ...
                  return (
                    <ZenCard
                      key={b.id}
                      selected={selectedBirth?.id === b.id}
                      disabled={!canAfford}
                      onClick={() => canAfford && setSelectedBirth(b)}
                    >
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <ZenButton disabled={!selectedBirth} onClick={goNext}>
                    下一步
                  </ZenButton>
                </div>
              </div>
            ...
            {step === 2 && (
              <div className="space-y-2">
                {IDENTITIES.map((id) => {
                  const canAfford =
                    id.cost <= remaining + (selectedIdentity?.id === id.id ? identityCost : 0);
                  return (
                    <ZenCard
                      key={id.id}
                      selected={selectedIdentity?.id === id.id}
                      disabled={!canAfford}
                      onClick={() => canAfford && setSelectedIdentity(id)}
                    >
                      <div className="p-3 flex items-center gap-3">
                        <span className="text-xl">{id.icon}</span>
                        <div className="flex-1">
                          <p className="font-semibold text-[#1A1A1A]">{id.name}</p>
                          <p className="text-xs text-gray-500">{id.desc}</p>
                        </div>
                        <span className="text-sm text-gray-500">-{id.cost}点</span>
                        {selectedIdentity?.id === id.id && (
                          <Check className="w-4 h-4 text-[#8C2D19]" />
                        )}
                      </div>
                    </ZenCard>
                  );
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <ZenButton disabled={!selectedIdentity} onClick={goNext}>
                    下一步
                  </ZenButton>
                </div>
              </div>
              ...
              <div className="space-y-2">
                {ELEMENTS.map((el) => (
                  <div key={el} className="space-y-1">
                    <p className="text-sm font-semibold" style={{ color: ELEMENT_COLORS[el] }}>
                      {el}灵根
                    </p>
                    <div className="flex gap-1">
                      {QUALITIES.map((q) => {
                        const id = `${el}_${q}`;
                        const isSelected =
                          selectedRoot?.element === el && selectedRoot?.quality === q;
                ...
                {/* chaos */}
                <ZenCard
                  selected={selectedRoot?.element === "chaos"}
                  onClick={() =>
                    setSelectedRoot({ element: "chaos", quality: "凡品", multiplier: 0.2 })
                  }
                >
                  <div className="p-3 flex items-center gap-3">
                    <span className="text-xl">🌫️</span>
                    <div className="flex-1">
                      <p className="font-semibold text-[#1A1A1A]">五行杂灵根</p>
                      <p className="text-xs text-gray-500">五行俱全，凡品，0.2x</p>
                    </div>
                    {selectedRoot?.element === "chaos" && (
                      <Check className="w-4 h-4 text-[#8C2D19]" />
                    )}
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <ZenButton disabled={!selectedRoot} onClick={goNext}>
                    下一步
                  </ZenButton>
                </div>
              </div>
              ...
              <ZenCard>
                <div className="p-4 space-y-2">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          className="w-7 h-7 flex items-center justify-center rounded bg-[#FAF8F5] border border-gray-200 text-[#1A1A1A] hover:border-[#8C2D19] hover:text-[#8C2D19] transition-colors text-sm"
                        >
                          -
                        </button>
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      onClick={handleReset}
                      className="flex-1 py-2 rounded-lg bg-[#FAF8F5] border border-gray-200 text-xs text-gray-500 hover:border-[#8C2D19] hover:text-[#8C2D19] transition-colors"
                    >
                      重置
                    </button>
                    <button
              ...
              (streamingText !== null ? (
                <ZenCard>
                  <div className="p-5 space-y-3">
                    <p className="text-center text-xs tracking-[0.3em] text-gray-400">
                      天地初开 · 降生
                    </p>
                    <div className="text-sm leading-relaxed whitespace-pre-wrap text-[#1A1A1A] min-h-[8rem]">
                      {streamingText}
                    </div>
                ...
                <ZenCard>
                  <div className="p-4 space-y-3">
                    <div className="mb-1">
                      <p className="font-semibold text-[#1A1A1A] calligraphy text-lg">
                        确认<span className="vermilion-underline text-[#8C2D19]">道身</span>
                      </p>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">道号</span>
                      <span className="text-[#1A1A1A] font-medium">（降生时由天定）</span>
                    </div>
                    <div className="flex justify-between text-sm">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-[#1A1A1A] font-medium">
                        {selectedBirth?.name} · {selectedBirth?.points}天资点
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">身份</span>
                      <span className="text-[#1A1A1A] font-medium">{selectedIdentity?.name}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">灵根</span>
                      <span className="text-[#1A1A1A] font-medium">
                        {selectedRoot?.element === "chaos"
                          ? "五行杂灵根 · 凡品 (0.2x)"
                          : `${selectedRoot?.element}灵根 · ${selectedRoot?.quality} (${selectedRoot?.multiplier}x)`}
                      </span>
                    </div>
                    {selectedTalentIds.length > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">天赋</span>
                        <span className="text-[#1A1A1A] font-medium">
                          {selectedTalentIds
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-[#1A1A1A] font-medium">
                        {ATTR_DEFS.map((a) => `${a.icon}${attributes[a.key] || 0}`).join(" · ")}
                      </span>
                    </div>
                    <div className="flex justify-between text-sm border-t border-gray-100 pt-2">
                      <span className="text-gray-500">已用天资点</span>
                      <span className="text-[#8C2D19] font-bold">
                        {birthPoints - remaining}/{birthPoints}
                      </span>
                    </div>
                    <div className="flex gap-2 pt-2">
                      <ZenButton variant="outline" onClick={goBack}>
                      ...
                      <ZenButton disabled={loading} onClick={handleCreate}>
                        {loading ? "点化中..." : "踏入仙途"}
                      </ZenButton>
                    </div>
                  </div>
