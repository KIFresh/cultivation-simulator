...
interface Book { id: string; title: string; author: string; attr: string; gain: number; staminaCost: number; desc: string; }
interface ReadingEntry { bookId: string; title: string; finishedAt: string; }
interface Cultivator { id: string; name: string; stamina: number; }

const ATTR_LABEL: Record<string, string> = Object.fromEntries(ATTR_INFO.map((a) => [a.key, `${a.icon}${a.label}`]));
...
export default function ReadingPage() {
  ...
  const read = async (book: Book) => {
    ...
    if ((cultivator?.stamina ?? 0) < book.staminaCost) { toast.error("体力不足，无法研读"); return; }
    setBusy(book.id);
    try {
      ...
      if (!res.ok) { toast.error(data.error || "研读失败"); return; }
      ...
      <div className="relative z-10 max-w-lg w-full mx-auto p-4 space-y-3">
        ...
        <Card className="border-border bg-card shadow-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-foreground flex items-center gap-2"><BookOpen className="w-5 h-5 text-primary" /> 课外阅读</CardTitle>
          </CardHeader>
            ...
            <Card key={b.id} className="border-border bg-card shadow-md">
              <CardContent className="p-3 flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{b.author} · {b.desc}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">体力 {b.staminaCost} · {ATTR_LABEL[b.attr] ?? b.attr} +{b.gain}</p>
                </div>
                <Button size="sm" className="bg-primary hover:bg-[#B33A2A] text-white shrink-0" disabled={busy !== null || cultivator.stamina < b.staminaCost} onClick={() => read(b)}>
                  {busy === b.id ? "..." : "研读"}
                </Button>
              </CardContent>
            </Card>
        ...
        {log.length > 0 && (
          <Card className="border-border bg-card shadow-md">
            <CardHeader className="pb-1"><CardTitle className="text-muted-foreground text-xs">已读 {log.length} 本</CardTitle></CardHeader>
            <CardContent className="space-y-1 max-h-40 overflow-y-auto">
              {log.slice().reverse().map((e, i) => (
                <p key={i} className="text-xs text-foreground border-b border-muted pb-1 last:border-0">{e.title}</p>
              ))}
            </CardContent>
          </Card>
        )}
