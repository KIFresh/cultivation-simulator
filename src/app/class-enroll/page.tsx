...
interface ClassInfo { id: string; name: string; school: string; cost: number; attr: string; desc: string; }
interface EnrollInfo { classId: string; name: string; school: string; enrolledAt: string; }
interface Cultivator { id: string; name: string; gold: number; }

const ATTR_LABEL: Record<string, string> = Object.fromEntries(ATTR_INFO.map((a) => [a.key, a.label]));
...
export default function ClassEnrollPage() {
  ...
  const enroll = async (cls: ClassInfo) => {
    ...
    if ((cultivator?.gold ?? 0) < cls.cost) { toast.error("灵石不足，无法入学"); return; }
    setBusy(true);
    try {
      ...
      <div className="relative z-10 max-w-lg w-full mx-auto p-4 space-y-3">
        ...
        <Card className="border-border bg-card shadow-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-foreground flex items-center gap-2"><GraduationCap className="w-5 h-5 text-primary" /> 课外班</CardTitle>
          </CardHeader>
              ...
              <Card key={cls.id} className="border-border bg-card shadow-md">
                <CardContent className="p-3 flex items-center gap-3">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-muted-foreground">{cls.school} · {cls.desc}</p>
                    <p className="text-[11px] text-muted-foreground mt-0.5">花费 {cls.cost} · 增益 {ATTR_LABEL[cls.attr] ?? cls.attr}</p>
                  </div>
                  <Button size="sm" variant={enrolled ? "outline" : "default"} className={enrolled ? "border-border text-muted-foreground shrink-0" : "bg-primary hover:bg-[#B33A2A] text-white shrink-0"} disabled={busy || enrolled || cultivator.gold < cls.cost} onClick={() => enroll(cls)}>
                    {enrolled ? "在读" : "报名"}
                  </Button>
                </CardContent>
              </Card>
