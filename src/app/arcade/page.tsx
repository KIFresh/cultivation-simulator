...
interface ArcadeStats { played: number; wins: number; gold: number; }
interface ArcadeGame { id: string; name: string; cost: number; maxPrize: number; desc: string; }
...
export default function ArcadePage() {
  ...
  const play = async (game: ArcadeGame) => {
    ...
    if ((cultivator?.gold ?? 0) < game.cost) { toast.error("灵石不足，无法投币"); return; }
    setBusy(game.id);
    try {
      ...
      if (!res.ok) { toast.error(data.error || "游玩失败"); return; }
      ...
      <div className="relative z-10 max-w-lg w-full mx-auto p-4 space-y-3">
        ...
        <Card className="border-border bg-card shadow-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-foreground flex items-center gap-2"><Gamepad2 className="w-5 h-5 text-primary" /> 街机厅</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs text-muted-foreground">
              <div><p className="text-foreground font-bold">{stats.played}</p>游玩</div>
              <div><p className="text-foreground font-bold">{stats.wins}</p>胜场</div>
              <div><p className="text-foreground font-bold">{stats.gold}</p>净灵石</div>
            </div>
          </CardContent>
        </Card>
            ...
            <Card key={g.id} className="border-border bg-card shadow-md">
              <CardContent className="p-3 flex items-center gap-3">
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground mt-0.5">花费 {g.cost} · 最高奖 {g.maxPrize}</p>
                </div>
                <Button size="sm" className="bg-primary hover:bg-[#B33A2A] text-white shrink-0" disabled={busy !== null || cultivator.gold < g.cost} onClick={() => play(g)}>
                  {busy === g.id ? "..." : "投币"}
                </Button>
              </CardContent>
            </Card>
          ))}
          {games.length === 0 && <p className="text-muted-foreground text-xs text-center py-2">暂无游艺项目</p>}
        </div>
      </div>
    </main>
  );
