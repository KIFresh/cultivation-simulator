...
interface UpkeepCost {
  petId: string;
  level: number;
  cost: { low: number; grass: number };
}
interface UpgradeCost {
  petId: string;
  level: number;
  cost: { mid: number; grass: number } | null;
}
        ...
        setState((s) =>
          s
            ? {
                ...
                spiritStoneLow:
                  body.action === "hatch"
                    ? s.spiritStoneLow - s.hatchCost.amount
                    : s.spiritStoneLow,
              }
            : s
        );
        ...
        <Card className="border-border bg-card shadow-md">
          <CardHeader className="pb-2">
            <CardTitle className="text-foreground flex items-center gap-2">
              <PawPrint className="w-5 h-5 text-primary" /> 灵宠洞府
            </CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-3 gap-2 text-center text-xs">
            <div className="bg-muted rounded p-2">
              <p className="text-foreground font-bold">{state?.spiritStoneLow ?? 0}</p>
            ...
            <div className="bg-muted rounded p-2">
              <p className="text-foreground font-bold">{state?.spiritStoneMid ?? 0}</p>
              <p className="text-muted-foreground flex items-center justify-center gap-0.5">
                <Gem className="w-3 h-3" />
                中品
              </p>
            ...
            <div className="bg-muted rounded p-2">
              <p className="text-foreground font-bold">{state?.grass ?? 0}</p>
              <p className="text-muted-foreground flex items-center justify-center gap-0.5">
          ...
          <Card className="border-primary/30 bg-primary/5 shadow-md">
            <CardContent className="p-3 space-y-2">
              <p className="text-sm text-foreground flex items-center gap-1">
                <Egg className="w-4 h-4" /> 你拥有一枚灵宠蛋
              </p>
              <Input
              ...
              <Button
                ...
                disabled={busy !== null || (state?.spiritStoneLow ?? 0) < state.hatchCost.amount}
                onClick={() => call({ action: "hatch", name: name.trim() || undefined }, "hatch")}
              >
        ...
        <div className="space-y-2">
          {state?.pets.map((p) => {
            ...
            const canUpgrade =
              ...
              !!up?.cost &&
              state.spiritStoneMid >= up.cost.mid &&
              state.grass >= up.cost.grass;
            const canUpkeep =
              upc && state.spiritStoneLow >= upc.cost.low && state.grass >= upc.cost.grass;
            return (
                ...
                <CardContent className="p-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    {p.level < (state.maxLevel ?? 5) && (
                      <Button
                        size="sm"
                        className="flex-1 bg-primary hover:bg-[#B33A2A] text-white"
                        disabled={busy !== null || !canUpgrade}
                        onClick={() => call({ action: "upgrade", petId: p.id }, "up_" + p.id)}
                      >
                        {busy === "up_" + p.id
                          ? "培育中..."
                          : up?.cost
                            ? `培育（${up.cost.mid}中品·${up.cost.grass}草）`
                            : "已满级"}
                      </Button>
                    )}
                    <Button
                      ...
                      disabled={busy !== null || !canUpkeep}
                      onClick={() => call({ action: "upkeep" }, "upkeep")}
                    >
                      {busy === "upkeep"
                        ? "养护中..."
                        : upc
                          ? `养护（${upc.cost.low}下品·${upc.cost.grass}草）`
                          : "养护"}
                    </Button>
                  </div>
                </CardContent>
