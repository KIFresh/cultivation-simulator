// 储蓄罐：零花钱计算（学段基础值 × 父母亲密度 × 父母收入）与年化利息。
...
export function calcPocketMoney(stage: string | null | undefined, parents: ParentLike[]): PocketMoneyResult {
  ...
  const incomeLevel = sample ? sample.incomeLevel : 1;
