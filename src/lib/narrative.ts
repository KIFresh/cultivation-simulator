...
const SYSTEM_PROMPT_BASE = `你是一个修仙世界的叙事引擎，用自己的语言写仙侠故事，不要复述设定，只需自然地呈现世界。
...
export async function generateBirthNarrative(params: {
  ...
  const familyStr = params.family && params.family.length > 0
    ...
    : "";
  const prompt = `写一段修仙小说的出生叙事。

【角色】${params.cultivatorName}，年龄${params.age || 1}岁，灵根${params.spiritualRoot}，身份${params.identityName || "未知"}，世界${params.worldName || "修仙世界"}${familyStr}

要求：200-350字，叙事中提及家庭成员时必须使用上面给出的真实姓名
返回JSON：{"title":"标题","narrative":"正文","mood":"悟/奇/静/燃","hint":"寄语"}`;
  try {
    const text = await callAI({ systemPrompt: buildSystemPrompt(params.worldId), userPrompt: prompt, maxTokens: 500, temperature: 0.85 });
