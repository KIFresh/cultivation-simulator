...
describe('encounter-data', () => {
  ...
  describe('resolveHighRiskOutcome', () => {
    ...
    it('should use fallback 40% for unknown root', () => {
      ...
      // 0.35 < 0.4 → true
      expect(resolveHighRiskOutcome({ spiritualRoot: 'unknown' as any, realmIndex: 0, realmLevel: 1 })).toBe(true);
      vi.restoreAllMocks();
    });
  });
  ...
  describe('applyRewardEffects', () => {
    it('should apply cultivationExp effect', () => {
      ...
      const result = applyRewardEffects(effects, { cultivationExp: 100, totalExp: 500, stamina: 50 });
    ...
    it('should apply stamina effect clamped to [0, 100]', () => {
      const effects: RewardEffect[] = [
        { type: 'stamina', value: 30, label: '+30 体力' },
      ];
    ...
    it('should not go below 0 for cultivationExp', () => {
      ...
      const result = applyRewardEffects(effects, { cultivationExp: 50, totalExp: 500, stamina: 50 });
