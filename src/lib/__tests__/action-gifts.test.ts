...
describe("evaluateActionGift - 家人零花钱", () => {
  it("合格的年幼请求至少获得 1 金，不会因向下取整归零", () => {
    const result = askMoney({
      cultivatorAge: 4,
      targetFamily: { ...mother, intimacy: 20, incomeLevel: 0 },
      allowanceRemaining: 10,
    });
  ...
  it("按年龄、收入和亲密度增加可发金额", () => {
    const young = askMoney({
      cultivatorAge: 6,
      targetFamily: { ...mother, intimacy: 50, incomeLevel: 1 },
    });
    const olderAndWealthier = askMoney({
      cultivatorAge: 15,
      targetFamily: { ...mother, intimacy: 90, incomeLevel: 2 },
    });
  ...
  it("幼儿在合格条件下获得象征性零花钱，关系极差、粗暴语气和额度耗尽会拒绝", () => {
    ...
    expect(askMoney({ targetFamily: { ...mother, intimacy: 19 } }).reasonCode).toBe(
      "RELATION_TOO_LOW"
    );
  ...
  it("父母别名和监护人都能发放零花钱，非家人不能", () => {
    ...
    expect(askMoney({ targetFamily: { ...mother, relation: "监护人" } }).reasonCode).toBe(
      "GRANTED"
    );
    expect(askMoney({ targetFamily: { ...mother, relation: "同学" } }).reasonCode).toBe(
      "NO_FAMILY_TARGET"
    );
