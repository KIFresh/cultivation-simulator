...
describe('NarrativeEffectSchema', () => {
  ...
  it('接受合法 intimacy 效果', () => {
    const result = NarrativeEffectSchema.parse({ kind: 'intimacy', delta: 3, targetRelation: '母亲' });
  ...
  it('拒绝非法 intimacy delta（超出 -20~20）', () => {
    expect(() => NarrativeEffectSchema.parse({ kind: 'intimacy', delta: 50, targetRelation: '母亲' })).toThrow();
  });

  it('接受合法 attrExp 效果', () => {
    const result = NarrativeEffectSchema.parse({ kind: 'attrExp', values: { root: 15, spirit: 10 } });
  ...
  it('拒绝已知但非法 attrExp 字段（拼写错误）', () => {
    expect(() => NarrativeEffectSchema.parse({
      kind: 'attrExp', values: { rooot: 5 },
    })).toThrow();
  });

  it('接受合法 storyEntry 效果', () => {
    const result = NarrativeEffectSchema.parse({
      kind: 'storyEntry', title: '奇遇', narrative: '路边捡到一块灵石',
    });
...
describe('extractEffects', () => {
  ...
  it('从 narrative 响应中提取', () => {
    const result = extractEffects({ narrative: '...', mood: '静', effects: [{ kind: 'stamina', delta: -3 }] });
    expect(result).toHaveLength(1);
  });
  ...
  it('过滤非法效果', () => {
    const result = extractEffects({ effects: [{ kind: 'gold', delta: 5 }, { kind: 'invalid', delta: 1 }] });
    expect(result).toHaveLength(1);
    expect(result[0]).toEqual({ kind: 'gold', delta: 5 });
  });
});
...
describe('clampEffect', () => {
  ...
  it('gold 不降低到负数以下', () => {
    expect(clampEffect({ kind: 'gold', delta: -50 }, { ...config, currentGold: 30 })).toEqual({ kind: 'gold', delta: -30 });
  });
  ...
  it('health 不超出 [0, maxHealth]', () => {
    expect(clampEffect({ kind: 'health', delta: -200 }, { ...config, currentHealth: 50 })).toEqual({ kind: 'health', delta: -50 });
  });

  it('intimacy 不超出 [0, 100]', () => {
    expect(clampEffect({ kind: 'intimacy', delta: 60, targetRelation: '母亲' }, { ...config, currentIntimacy: 50, maxIntimacyAbsDelta: 60 })).toEqual({ kind: 'intimacy', delta: 50, targetRelation: '母亲' });
    expect(clampEffect({ kind: 'intimacy', delta: -60, targetRelation: '母亲' }, { ...config, currentIntimacy: 50, maxIntimacyAbsDelta: 60 })).toEqual({ kind: 'intimacy', delta: -50, targetRelation: '母亲' });
  });

  it('mindDemon 不降低到负数以下', () => {
    expect(clampEffect({ kind: 'mindDemon', delta: -10 }, { ...config, currentMindDemon: 3 })).toEqual({ kind: 'mindDemon', delta: -3 });
  });
});
...
describe('aggregateEffects', () => {
  ...
  it('clampEffect 边界：无 currentStamina 时仅做绝对限幅', () => {
    const config: ClampConfig = { maxGold: 200, maxStamina: 100, maxHealth: 100, maxMindDemon: 100 };
    expect(clampEffect({ kind: 'stamina', delta: 200 }, config)).toEqual({ kind: 'stamina', delta: 50 });
  });

  it('clampEffect 边界：无 currentHealth 时仅做绝对限幅', () => {
    const config: ClampConfig = { maxGold: 200, maxStamina: 100, maxHealth: 100, maxMindDemon: 100 };
    expect(clampEffect({ kind: 'health', delta: 200 }, config)).toEqual({ kind: 'health', delta: 100 });
  });

  it('clampEffect 边界：无 currentMindDemon 时仅做绝对限幅', () => {
    const config: ClampConfig = { maxGold: 200, maxStamina: 100, maxHealth: 100, maxMindDemon: 100 };
    expect(clampEffect({ kind: 'mindDemon', delta: 200 }, config)).toEqual({ kind: 'mindDemon', delta: 100 });
  });
...
describe('storyEntry 压缩', () => {
  it('超过 50 条时通过 applyEffects 压缩', async () => {
    ...
    const entries = [];
    for (let i = 0; i < 40; i++) {
      entries.push({ title: `重要事件${i}`, narrative: `重要描述${i}`, important: true, createdAt: new Date(1000 * i).toISOString() });
    }
    for (let i = 0; i < 20; i++) {
      entries.push({ title: `普通事件${i}`, narrative: `普通描述${i}`, important: false, createdAt: new Date(1000 * 100 + 1000 * i).toISOString() });
    }
...
describe('applyEffects', () => {
  ...
  it('attrExp 正确合并到 JSON 字段', async () => {
    ...
    mockTx.cultivator.update.mockResolvedValue({});
    await applyEffects([{ kind: 'attrExp', values: { root: 10, luck: 5 } }], mockTx as any, baseCtx);
    const updateCall = mockTx.cultivator.update.mock.calls[0][0];
    ...
    expect(mockTx.familyMember.createMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.arrayContaining([
          expect.objectContaining({ occupation: '铁匠' }),
        ]),
      }),
    );
  });
