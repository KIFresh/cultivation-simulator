...
describe("cleanNarrativeStream", () => {
  it("剥离开头的 ```json 栅栏并提取 narr 字段", () => {
    expect(cleanNarrativeStream('```json\n{"type":"ACTION","narrative":"你好"}')).toBe("你好");
  });
  ...
  it("剥离开头的 <function> 标签并提取 narr", () => {
    expect(cleanNarrativeStream('<function>{"type":"ACTION","narr":"你好"}')).toBe("你好");
  });

  it("剥离开头的 <function_call> 与结尾 </function_call>", () => {
    expect(cleanNarrativeStream('<function_call>\n{"narr":"你好"}\n</function_call>')).toBe("你好");
  });

  it("剥离开头的 <thinking> 标签", () => {
    expect(cleanNarrativeStream('<thinking>{"narrative":"你好"}')).toBe("你好");
  });
  ...
  it("正文中间的同类字符不受影响", () => {
    expect(cleanNarrativeStream("他说：「```不是栅栏```」这是正文")).toBe(
      "他说：「```不是栅栏```」这是正文"
    );
  });
  ...
  it("从完整 JSON 对象中提取 narr 字段", () => {
    const json =
      '{"type":"ACTION","title":"春日的漫步","narr":"春日风暖送城郭的街角。","mood":"静","hint":"继续探索"}';
  ...
  it("长叙事文本完整提取", () => {
    const longNarr =
      "春日风暖送城郭的街角，带来了些许飘絮和泥土的芬芳。三岁的穿着轻便的蓑衣，在小区附近的公园里踏青自娱。他不懂什么大道理，只是单纯地被路边盛开的迎春花吸引，试着小脑袋凑近闻了闻那股淡淡的清香。墙里发出无意义的咿呀声。在这平凡的春日午后，没有灵气的波动，也没有超凡的迹象。只有孩童对世界最原始的探索与天真。";
