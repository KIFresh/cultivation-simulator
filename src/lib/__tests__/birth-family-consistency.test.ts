...
describe("validateBirthConsistency", () => {
  ...
  it("正常家庭：正文含所有姓名关系，无错误", () => {
    const narrative =
      '寒冬腊月，李家的产房里传来一声啼哭。父亲李建国焦急地在门外踱步，母亲王秀兰躺在床上，虚弱地看着 newborn。接生婆笑着说："是个男孩！"李建国给孩子取名李逍遥。';
  ...
  it("suggestedName 未出现在正文中时报告错误", () => {
    const narrative =
      "产房里传来啼哭声。父亲李建国焦急踱步，母亲王秀兰躺在床上。接生婆说是个男孩。";
  ...
  it("身份冲突：正文称某人母亲，JSON 称姐姐", () => {
    const narrative =
      "产房里，母亲王秀兰抱着刚出生的婴儿。父亲李建国在一旁欣慰地笑着。给孩子取名李雨。";
    const family: BirthFamilyMember[] = [
      { relation: "父亲", name: "李建国", age: 38, alive: true },
  ...
  it("已故成员正常通过", () => {
    const narrative =
      "祖母张翠花虽已过世，但家人仍念着她的好。父亲李建国和母亲王秀兰喜迎新生儿。给孩子取名李思。";
    const family: BirthFamilyMember[] = [
      { relation: "父亲", name: "李建国", age: 38, alive: true },
      { relation: "母亲", name: "王秀兰", age: 36, alive: true },
  ...
  it("祖辈同住正常通过", () => {
    const narrative =
      "爷爷李铁柱和奶奶张翠花在客厅里喜笑颜开。父亲李建国和母亲王秀兰在卧室照顾婴儿。给孩子取名李明珠。";
    const family: BirthFamilyMember[] = [
      { relation: "父亲", name: "李建国", age: 38, alive: true },
      { relation: "母亲", name: "王秀兰", age: 36, alive: true },
  ...
  it("兄弟姐妹正常通过", () => {
    const narrative =
      "姐姐李小娟趴在床边好奇地看着弟弟。父亲李建国笑着说这是你妹妹。母亲王秀兰轻声哄着婴儿。给孩子取名李小雨。";
    const family: BirthFamilyMember[] = [
      { relation: "父亲", name: "李建国", age: 38, alive: true },
      { relation: "母亲", name: "王秀兰", age: 36, alive: true },
