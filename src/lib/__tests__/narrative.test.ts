...
describe("narrative", () => {
  ...
  describe("appendToSummary", () => {
    ...
    it("should append to existing summary", () => {
      const result = appendToSummary("【之前】之前内容", {
        title: "新事件",
        narrative: "新叙事内容",
      });
    ...
    it("should truncate narrative to 60 chars", () => {
      const longNarrative =
        "这是一段很长的叙事内容，目的是测试截断功能是否正常工作，超过六十个字符后应该被截断并加上省略号，这是额外加长以确保超过六十字符限制";
  ...
  describe("validateBirthConsistency", () => {
    ...
    it("should detect invalid age", () => {
      ...
      const family = [{ relation: "父亲", name: "李建国", age: -1, alive: true }];
    ...
    it("should detect non-boolean alive", () => {
      ...
      const family = [{ relation: "父亲", name: "李建国", age: 30, alive: 1 as any }];
      const errors = validateBirthConsistency(narrative, "小明", family);
...
describe("extractJson - AI 拼写错误兼容", () => {
  ...
  it("应兼容 narrable 拼写", () => {
    const result = extractJson(
      '{"type":"BIRTH","narrable":"凌晨三点，医院产房，祖父李守仁坐在长椅上，手里摩挲着一本泛黄的族谱。"}',
      { narrative: "" }
    );
    expect(result.narrative).toBe(
      "凌晨三点，医院产房，祖父李守仁坐在长椅上，手里摩挲着一本泛黄的族谱。"
    );
  });

  it("应兼容 narrrative 拼写", () => {
    const result = extractJson(
      '{"type":"BIRTH","narrrative":"双r叙事，这是一段很长的中文叙事文本，用来测试AI拼写错误的兼容性。"}',
      { narrative: "" }
    );
    expect(result.narrative).toBe(
      "双r叙事，这是一段很长的中文叙事文本，用来测试AI拼写错误的兼容性。"
    );
  });

  it("应兼容正确拼写 narrative", () => {
    const result = extractJson(
      '{"type":"BIRTH","narrative":"正确的叙事，这是一段很长的中文叙事文本。"}',
      { narrative: "" }
    );
  ...
  it("无 narr 字段时回退到 content", () => {
    const result = extractJson(
      '{"type":"BIRTH","content":"content叙事，这是一段很长的中文叙事文本。"}',
      { narrative: "" }
    );
  ...
  it("无 narr/content 字段时找第一个长中文字段", () => {
    const result = extractJson(
      '{"type":"BIRTH","description":"这是一段很长的中文叙事文本，用来测试AI返回的字段名完全不匹配时能否正确识别。"}',
      { narrative: "" }
    );
    expect(result.narrative).toBe(
      "这是一段很长的中文叙事文本，用来测试AI返回的字段名完全不匹配时能否正确识别。"
    );
  });
