...
describe('cultivation-data', () => {
  ...
  describe('calculateActionExp', () => {
    ...
    it('should halve exp when injured', () => {
      const normal = calculateActionExp('MEDITATE', '天灵根', { spirit: 0 }, [], 0, undefined, 1, 0);
      const injured = calculateActionExp('MEDITATE', '天灵根', { spirit: 0 }, [], 0, undefined, 1, 1);
  ...
  describe('calculateSchoolRank', () => {
    ...
    it('should return 名校 for high attributes', () => {
      expect(calculateSchoolRank(18, { insight: 20, mind: 15, root: 10, spirit: 10, luck: 10, charm: 10 })).toBe('名校');
    });
  });
  ...
  describe('calculateYearlyAttributeGrowth', () => {
    ...
    it('should grow attributes for children', () => {
      const result = calculateYearlyAttributeGrowth(0, 1, { root: 5, spirit: 5, insight: 5, luck: 5, charm: 5, mind: 5 });
  ...
  describe('parseOccupationFromNarrative', () => {
    it('should detect 辍学', () => expect(parseOccupationFromNarrative('他决定辍学', '')).toBe('辍学'));
    it('should detect 炼丹师', () => expect(parseOccupationFromNarrative('开始炼丹', '')).toBe('炼丹师'));
    it('should return null for no match', () => expect(parseOccupationFromNarrative('平凡的一天', '')).toBeNull());
  });
  ...
  describe('applyActivityEffects', () => {
    it('should apply attribute growth', () => {
      const activity = { id: 'exercise', name: '锻炼', icon: '🏃', description: '', staminaCost: 3, goldDelta: 0, attrGrowth: [['root', 1.5] as [string, number]], minAge: 3 };
