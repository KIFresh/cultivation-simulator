...
describe("calculateAnnualFamilyAllowance", () => {
  it("按年龄、最高家庭收入与平均亲密度生成年度额度", () => {
    expect(
      calculateAnnualFamilyAllowance(10, [
  ...
  it("没有在世监护人时不生成额度，幼儿在亲密且宽裕家庭可有象征性额度", () => {
    ...
    expect(
      calculateAnnualFamilyAllowance(3, [{ intimacy: 90, incomeLevel: 2 }])
  ...
  it("使用统一 HouseholdIncome 档位而非单个家人的旧收入", () => {
    ...
    expect(calculateAnnualFamilyAllowance(12, [{ intimacy: 60, incomeLevel: 0 }], household)).toBe(
      calculateAnnualFamilyAllowance(12, [{ intimacy: 60, incomeLevel: 2 }], household)
    );
  });
});
